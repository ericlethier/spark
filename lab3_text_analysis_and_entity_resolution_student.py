...
def simpleTokenize(string):
    ...
    """
    token_list = re.split(split_regex, string.lower(), flags=re.IGNORECASE)
...
def tokenize(string):
    ...
    token_list = re.split(split_regex, string.lower(), flags=re.IGNORECASE)
    token_list = filter(None, token_list)
    token_list = filter(lambda token : token not in stopwords, token_list)
    return token_list
...
amazonRecToToken = amazonSmall.map(lambda list : (list[0], tokenize(list[1])))
googleRecToToken = googleSmall.map(lambda list : (list[0], tokenize(list[1])))
...
def countTokens(vendorRDD):
    ...
    """
    count_tokens = vendorRDD.map(lambda (id, tokens) : (len(tokens))).collect()
    return reduce(lambda x,y: x+y, count_tokens)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (id, tokens) : (id, tokens)).takeOrdered(1, key= lambda(k,v) : -len(v))
...
def tf(tokens):
    ...
    for token in dict:
        dict[token] = float(dict[token]) / float(len(tokens))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (uid, tokens) : set(tokens) )
    tokenCountPairTuple = uniqueTokens.map(lambda token : (token, 1))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(add)
    return (tokenSumPairTuple.map(lambda (token, count) : (token, float(N)/float(count))))
...
def tfidf(tokens, idfs):
    ...
    tfIdfDict = {}
    for uid in tokens:
        tfIdfDict[uid] = tfs[uid] * idfs[uid]
...
def dotprod(a, b):
    ...
    dotproduct = sum(a[k] * b[k] for k in keys)
    return dotproduct
...
cosineList = trueDupsRDD.map(lambda (x,(y,z)) : y).collect()
avgSimDups = float(sum(cosineList)) / float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard))

cosineListN = nonDupsRDD.map(lambda (x,y) : y).collect()
nonDupsCount = nonDupsRDD.count()
avgSimNon = float(sum(cosineListN)) / float(nonDupsCount)
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda list : (list[0], tokenize(list[1])))
googleFullRecToToken = google.map(lambda list : (list[0], tokenize(list[1])))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (uid, dico) : (uid, norm(dico))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda (uid, dico) : (uid, norm(dico))).collectAsMap()
...
def swap(record):
    ...
    token = record[0]
    amazonId = record[1][0]
    googleUrl = record[1][1]
    return ((amazonId, googleUrl), token)

#print amazonInvPairsRDD.take(2)

commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD)
                .map(lambda record : swap(record))
...
# TODO: Replace <FILL IN> with appropriate code
amazonWeightsBroadcast = sc.broadcast(amazonWeights)
googleWeightsBroadcast = sc.broadcast(googleWeights)
