...
topTenErrURLs = endpointSum.takeOrdered(10, key= lambda (k, v) : -v)
...
uniqueHosts = hosts.map(lambda uniquehost : (uniquehost ,1)).reduceByKey(add)
...
dayGroupedHosts = dayToHostPairTuple.map(lambda day_host : (day_host ,1)).reduceByKey(add)

dayHostCount = dayGroupedHosts.reduceByKey(add)

dailyHosts = dayHostCount.map(lambda (day_host, count) : (day_host[0], 1)).reduceByKey(add).sortByKey()
...
groupedByDay = dayAndHostTuple.map(lambda day_host : (day_host[0] ,1)).reduceByKey(add)

sortedByDay = groupedByDay.sortByKey()

joinedRdd = sortedByDay.join(dailyHosts)

avgDailyReqPerHost = joinedRdd.map(lambda (x,(y,z)) : (x, y/z)).sortByKey()
...
tmp = badEndpoints.map(lambda endpoint : (endpoint ,1)).reduceByKey(add)

badUniqueEndpoints = tmp.map(lambda (x,y) : (x))
...
badEndpointsTop20 = badEndpointsSum.takeOrdered(20, key= lambda (k, v) : -v)
...
errHostsTop25 = errHostsSum.takeOrdered(25, key= lambda (k, v) : -v)
...
daysWithErrors404 = errDateSorted.map(lambda (day, sum) : (day)).collect()
errors404ByDay = errDateSorted.map(lambda (day, sum) : (sum)).collect()
...
topErrDate = errDateSorted.takeOrdered(5, lambda (day, sum) : (-sum))
...
hoursWithErrors404 = hourRecordsSorted.map(lambda (hour, sum) : (hour)).collect()
errors404ByHours = hourRecordsSorted.map(lambda (hour, sum) : (sum)).collect()
