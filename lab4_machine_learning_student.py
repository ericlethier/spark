...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    return (movie_id, (len(ratings), average_rating))
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda my_tuple : (my_tuple[1], [my_tuple[2]]))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda id_ratings : getCountsAndAverages(id_ratings))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda my_tuple : (my_tuple[1][1][1], my_tuple[1][0], my_tuple[1][1][0]))
                              )
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda my_tuple: my_tuple[2] > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    #print predictedReformattedRDD.take(1)
                                               
    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda (UserID, MovieID, Rating) : ((UserID, MovieID), Rating))
    #print actualReformattedRDD.take(1)

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .map(lambda (ids,ratings) : (ids, math.pow(ratings[0] - ratings[1],2) )))
    print squaredErrorsRDD.take(10)

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda (ids, squarrederrors) : squarrederrors).sum()
...
#userId, movieId, rating
# TODO: Replace <FILL IN> with appropriate code
trainingAvgRating = (trainingRDD.map(lambda (userId, movieId, rating) : (rating))
                                .sum()) / trainingRDD.count()
